...
def column_type(table_name, column_name):
    _type = [
        x["type"].python_type
        for x in insp.get_columns(table_name)
...
def upgrade():
    ...
    if "table_custom_score_profile_conditions" in tables:
        with op.batch_alter_table("table_custom_score_profile_conditions") as batch_op:
            batch_op.execute(
                "DROP INDEX IF EXISTS tablecustomscoreprofileconditions_profile_id"
            )
            batch_op.alter_column("profile_id", index=False)
            batch_op.execute(
                "DROP INDEX IF EXISTS ix_table_custom_score_profile_conditions_profile_id;"
            )
    ...
    with op.batch_alter_table("table_shows", recreate=should_recreate) as batch_op:
        if bind.engine.name == "postgresql":
            batch_op.execute(
                "ALTER TABLE table_shows DROP CONSTRAINT IF EXISTS table_shows_pkey CASCADE;"
            )
        batch_op.execute(
            sa.update(TableShows)
            .values({TableShows.profileId: None})
            .where(
                TableShows.profileId.not_in(sa.select(TableLanguagesProfiles.profileId))
            )
        )
        batch_op.create_primary_key(
            constraint_name="pk_table_shows", columns=["sonarrSeriesId"]
        )
        batch_op.create_foreign_key(
            constraint_name="fk_series_profileId_languages_profiles",
        ...
        batch_op.alter_column(
            column_name="tvdbId", existing_type=sa.INTEGER(), nullable=True
        )
        if column_exists("table_shows", "alternateTitles") and not column_exists(
            "table_shows", "alternativeTitles"
        ):
            batch_op.alter_column(
                column_name="alternateTitles", new_column_name="alternativeTitles"
            )
        ...
        if not constraint_exists("table_shows", "unique_table_shows_path"):
            batch_op.create_unique_constraint(
                constraint_name="unique_table_shows_path", columns=["path"]
            )
    ...
    with op.batch_alter_table("table_episodes") as batch_op:
        if bind.engine.name == "postgresql":
            batch_op.execute(
                "ALTER TABLE table_episodes DROP CONSTRAINT IF EXISTS table_episodes_pkey CASCADE;"
            )
        batch_op.execute(
            sa.delete(TableEpisodes).where(
                TableEpisodes.sonarrSeriesId.not_in(
                    sa.select(TableShows.sonarrSeriesId)
                )
            )
        )
        batch_op.alter_column(
            column_name="sonarrSeriesId", existing_type=sa.INTEGER(), nullable=True
        )
        batch_op.create_primary_key(
            constraint_name="pk_table_episodes", columns=["sonarrEpisodeId"]
        )
        batch_op.create_foreign_key(
            constraint_name="fk_sonarrSeriesId_episodes",
    ...
    with op.batch_alter_table("table_history") as batch_op:
        batch_op.execute(
            sa.delete(TableHistory).where(
                TableHistory.sonarrEpisodeId.not_in(
        ...
        batch_op.execute(
            sa.delete(TableHistory).where(
                TableHistory.sonarrSeriesId.not_in(sa.select(TableShows.sonarrSeriesId))
            )
        )
        batch_op.alter_column(
            column_name="sonarrEpisodeId", existing_type=sa.INTEGER(), nullable=True
        )
        batch_op.alter_column(
            column_name="sonarrSeriesId", existing_type=sa.INTEGER(), nullable=True
        )
        batch_op.create_foreign_key(
            constraint_name="fk_sonarrEpisodeId_history",
        ...
        batch_op.create_foreign_key(
            constraint_name="fk_sonarrSeriesId_history",
            referent_table="table_shows",
            local_cols=["sonarrSeriesId"],
            remote_cols=["sonarrSeriesId"],
            ondelete="CASCADE",
        ...
        if column_type("table_history", "score") == str:  # noqa: E721
            batch_op.alter_column(
                column_name="score", existing_type=sa.Text, type_=sa.Integer
            )
        if column_type("table_history", "timestamp") == int:  # noqa: E721
            table_history_timestamp_altered = True
            batch_op.alter_column(
                column_name="timestamp", existing_type=sa.Integer, type_=sa.DateTime
            )
    with op.batch_alter_table("table_history") as batch_op:
        ...
        if table_history_timestamp_altered:
            batch_op.execute(
                sa.text(
                    "UPDATE table_history SET timestamp = datetime(timestamp, 'unixepoch')"
                )
            )
    ...
    with op.batch_alter_table("table_blacklist") as batch_op:
        batch_op.execute(
            sa.delete(TableBlacklist).where(
                TableBlacklist.sonarr_episode_id.not_in(
                    sa.select(TableEpisodes.sonarrEpisodeId)
                )
            )
        )
        batch_op.execute(
            sa.delete(TableBlacklist).where(
                TableBlacklist.sonarr_series_id.not_in(
                    sa.select(TableShows.sonarrSeriesId)
                )
            )
        )
        batch_op.create_foreign_key(
            constraint_name="fk_sonarrEpisodeId_blacklist",
            referent_table="table_episodes",
        ...
        batch_op.create_foreign_key(
            constraint_name="fk_sonarrSeriesId_blacklist",
            referent_table="table_shows",
        ...
        if column_type("table_blacklist", "timestamp") == int:  # noqa: E721
            ...
            batch_op.alter_column(
                column_name="timestamp", existing_type=sa.DATETIME(), nullable=True
            )
    with op.batch_alter_table("table_blacklist") as batch_op:
        # must be run after alter_column as been committed
        if table_blacklist_timestamp_altered:
            batch_op.execute(
                sa.text(
                    "UPDATE table_blacklist SET timestamp = datetime(timestamp, 'unixepoch')"
                )
            )
    ...
    with op.batch_alter_table("table_shows_rootfolder") as batch_op:
        ...
        if bind.engine.name == "postgresql":
            batch_op.execute(
                "ALTER TABLE table_shows_rootfolder DROP CONSTRAINT IF EXISTS "
        ...
        batch_op.create_primary_key(
            constraint_name="pk_table_shows_rootfolder", columns=["id"]
        )
    ...
    with op.batch_alter_table("table_movies", recreate=should_recreate) as batch_op:
        if bind.engine.name == "postgresql":
            batch_op.execute(
                "ALTER TABLE table_movies DROP CONSTRAINT IF EXISTS table_movies_pkey CASCADE;"
            )
        batch_op.execute(
            sa.update(TableMovies)
            .values({TableMovies.profileId: None})
            .where(
                TableMovies.profileId.not_in(
                    sa.select(TableLanguagesProfiles.profileId)
                )
            )
        )
        batch_op.create_primary_key(
            constraint_name="pk_table_movies", columns=["radarrId"]
        )
        batch_op.create_foreign_key(
            constraint_name="fk_movies_profileId_languages_profiles",
            referent_table="table_languages_profiles",
            local_cols=["profileId"],
            remote_cols=["profileId"],
            ondelete="SET NULL",
        ...
        if not constraint_exists("table_movies", "unique_table_movies_path"):
            batch_op.create_unique_constraint(
                constraint_name="unique_table_movies_path", columns=["path"]
            )
        if not constraint_exists("table_movies", "unique_table_movies_tmdbId"):
            batch_op.create_unique_constraint(
                constraint_name="unique_table_movies_tmdbId", columns=["tmdbId"]
            )
    ...
    with op.batch_alter_table("table_history_movie") as batch_op:
        batch_op.execute(
            sa.delete(TableHistoryMovie).where(
                TableHistoryMovie.radarrId.not_in(sa.select(TableMovies.radarrId))
            )
        )
        batch_op.alter_column(
            column_name="radarrId", existing_type=sa.INTEGER(), nullable=True
        )
        batch_op.create_foreign_key(
            constraint_name="fk_radarrId_history_movie",
    ...
    with op.batch_alter_table("table_history_movie") as batch_op:
        ...
        if table_history_movie_timestamp_altered:
            batch_op.execute(
                sa.text(
                    "UPDATE table_history_movie SET timestamp = datetime(timestamp, 'unixepoch')"
                )
            )
    ...
    with op.batch_alter_table("table_blacklist_movie") as batch_op:
        batch_op.execute(
            sa.delete(TableBlacklistMovie).where(
                TableBlacklistMovie.radarr_id.not_in(sa.select(TableMovies.radarrId))
            )
        )
        batch_op.create_foreign_key(
            constraint_name="fk_radarrId_blacklist_movie",
            referent_table="table_movies",
    ...
    with op.batch_alter_table("table_blacklist_movie") as batch_op:
        ...
        if table_blacklist_movie_timestamp_altered:
            batch_op.execute(
                sa.text(
                    "UPDATE table_blacklist_movie SET timestamp = datetime(timestamp, 'unixepoch')"
                )
            )
    ...
    with op.batch_alter_table("table_movies_rootfolder") as batch_op:
        ...
        if bind.engine.name == "postgresql":
            batch_op.execute(
                "ALTER TABLE table_movies_rootfolder DROP CONSTRAINT IF EXISTS "
        ...
        batch_op.create_primary_key(
            constraint_name="pk_table_movies_rootfolder", columns=["id"]
        )
