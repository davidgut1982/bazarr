...
def upgrade():
    ...
    if not table_exists(insp, "provider_hub_catalog_entries"):
        op.create_table(
            ...
            sa.Column(
                "source_id",
                sa.Integer(),
                sa.ForeignKey("provider_hub_catalog_sources.id", ondelete="CASCADE"),
            ),
        ...
        op.create_index(
            "ix_provider_hub_catalog_entries_source_id",
            "provider_hub_catalog_entries",
            ["source_id"],
        )
        op.create_index(
            "ix_provider_hub_catalog_entries_provider_id",
            "provider_hub_catalog_entries",
            ["provider_id"],
        )
    ...
    if not table_exists(insp, "provider_hub_secrets"):
        ...
        op.create_index(
            "ix_provider_hub_secrets_provider_id",
            "provider_hub_secrets",
            ["provider_id"],
        )
    ...
    if not table_exists(insp, "provider_hub_jobs"):
        ...
        op.create_index(
            "ix_provider_hub_jobs_provider_id", "provider_hub_jobs", ["provider_id"]
        )
    ...
    if not table_exists(insp, "provider_hub_install_events"):
        ...
        op.create_index(
            "ix_provider_hub_install_events_provider_id",
            "provider_hub_install_events",
            ["provider_id"],
        )
        op.create_index(
            "ix_provider_hub_install_events_job_id",
            "provider_hub_install_events",
            ["job_id"],
        )
