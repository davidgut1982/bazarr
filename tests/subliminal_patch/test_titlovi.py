...
def test_list_subtitles_and_download_from_pack(
    ...
    with open(os.path.join(data, "titlovi_gettoken_response.json"), "rb") as f:
        response = f.read()
        requests_mock.post(
            "https://kodi.titlovi.com/api/subtitles/gettoken?username=user1&password=pass1&json=True",
            content=response,
        )
    ...
    with open(os.path.join(data, "titlovi_search_response.json"), "rb") as f:
        response = f.read()
        requests_mock.get(
            "https://kodi.titlovi.com/api/subtitles/search?token=asdf1234&userid=111&&query=la femme nikita&lang=Srpski&json=True",
            content=response,
        )
    ...
    with open(os.path.join(data, "titlovi_some_subtitle_pack.zip"), "rb") as f:
        response = f.read()
        requests_mock.get(
            "https://titlovi.com/download/?type=1&mediaid=81022", content=response
        )
    ...
    with TitloviProvider("user1", "pass1") as provider:
        ...
        with open(os.path.join(data, "titlovi_some_subtitle_pack.zip"), "rb") as f:
            ...
            subtitle_content = fix_line_ending(
                archive.read("La Femme Nikita - 01x13 - Recruit.srt")
            )
