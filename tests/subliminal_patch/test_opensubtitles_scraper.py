...
from subliminal_patch.providers.opensubtitles_scraper import (
    OpenSubtitlesScraperMixin,
    _LANG_3_TO_2,
)
from subliminal_patch.providers.opensubtitles import (
...
class TestSelectBestResult:
    ...
    def test_no_results(self, provider):
        assert (
            provider._select_best_result([], "tt0816692", "Interstellar", 2014) is None
        )
...
class TestQueryScraper:
    ...
    def test_movie_search_flow(
        self, mock_request, provider, mock_movie, search_result_movie, subtitle_response
    ):
        ...
        subtitles = provider._query_scraper(
            mock_movie, languages, imdb_id="tt0816692", query=["Interstellar"]
        )
    ...
    @patch.object(FakeProvider, "_scraper_request")
    def test_tv_search_flow(
        self, mock_request, provider, mock_episode, search_result_tv, subtitle_response
    ):
    ...
    @patch.object(FakeProvider, "_scraper_request")
    def test_passes_video_title_as_query(
        self, mock_request, provider, mock_movie, search_result_movie, subtitle_response
    ):
...
class TestParseV1Subtitles:
    def test_builds_quoted_movie_name_for_episodes(
        self, provider, mock_episode, subtitle_response
    ):
        search_result = {
            "title": "Breaking Bad",
            "year": 2008,
...
class TestDownloadSubtitle:
    ...
    def test_download_with_url(self, mock_request, provider):
        ...
        subtitle.scraper_download_url = (
            "https://www.opensubtitles.org/en/subtitles/12345"
        )
