...
def test_e2e_search_then_download_then_stream(app, tmp_path, monkeypatch):
    ...
    with (
        patch("compat.local_subs._resolve_media", side_effect=_resolve),
        ...
        patch(
            "compat.service.list_all_subtitles_parallel", return_value={MagicMock(): []}
        ),
    ):
        ...
        # 1. Search
        r = client.get(
            "/api/v1/subtitles?imdb_id=tt1375666&languages=en",
        ...
        login_r = client.post("/api/v1/login", headers={"Api-Key": "a" * 32})
        token = login_r.get_json()["token"]

        # 3. Download
        d = client.post(
            "/api/v1/download",
            headers={"Api-Key": "a" * 32, "Authorization": f"Bearer {token}"},
        ...
        s = client.get(f"/api/v1{stream_path}", headers={"Api-Key": "a" * 32})
