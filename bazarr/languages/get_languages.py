...
def load_language_in_db():
    ...
    langs = [
        {"code3": lang.alpha_3, "code2": lang.alpha_2, "name": lang.name, "enabled": 0}
    ...
    database.execute(
        insert(TableSettingsLanguages).values(langs).on_conflict_do_nothing()
    )

    # Update standard languages with code3b if available
    langs = [
        {"code3b": lang.bibliographic, "code3": lang.alpha_3}
        for lang in pycountry.languages
...
def language_from_alpha3(lang):
    return next(
        (
            ...
            if lang[:3] in [item["code3"], item["code3b"]]
        ),
        None,
    )
...
def get_language_set():
    languages = database.execute(
        select(TableSettingsLanguages.code3).where(TableSettingsLanguages.enabled == 1)
    ).all()
