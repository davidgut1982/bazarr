...
api_ns_movies_wanted = Namespace(
    "Movies Wanted", description="List movies wanted subtitles"
)
...
class MoviesWanted(Resource):
    ...
    get_subtitles_language_model = api_ns_movies_wanted.model(
        "subtitles_language_model", subtitles_language_model
    )
    get_audio_language_model = api_ns_movies_wanted.model(
        "audio_language_model", audio_language_model
    )

    data_model = api_ns_movies_wanted.model(
        "wanted_movies_data_model",
        {
    ...
    get_response_model = api_ns_movies_wanted.model(
        "MovieWantedGetResponse",
        {
    ...
    def get(self):
        ...
        if length > 0:
            stmt = (
                stmt.order_by(TableMovies.radarrId.desc()).limit(length).offset(start)
            )
        ...
        count = database.execute(
            select(func.count()).select_from(TableMovies).where(wanted_condition)
        ).scalar()
