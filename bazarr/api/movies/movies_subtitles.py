...
api_ns_movies_subtitles = Namespace(
    "Movies Subtitles", description="Download, upload or delete movies subtitles"
)
...
class MoviesSubtitles(Resource):
    ...
    @api_ns_movies_subtitles.response(
    ...
    def post(self):
        ...
        movieInfo = database.execute(
            select(TableMovies.path, TableMovies.audio_language).where(
    ...
    def delete(self):
        ...
        radarrId = args.get("radarrid")
        movieInfo = database.execute(
            select(TableMovies.path).where(TableMovies.radarrId == radarrId)
        ).first()
