...
api_ns_system_account = Namespace(
    "System Account", description="Login or logout from Bazarr UI"
)
...
def _record_failed_attempt(ip):
    with _login_lock:
        ...
        if count >= _MAX_ATTEMPTS:
            logging.warning(
                f"Login rate limit triggered for {ip} after {count} failed attempts"  # noqa: G004
            )
...
class SystemAccount(Resource):
    ...
    post_request_parser.add_argument(
        "username", type=str, required=False, help="Bazarr username"
    )
    post_request_parser.add_argument(
        "password", type=str, required=False, help="Bazarr password"
    )
    ...
    @api_ns_system_account.response(
        406, "Browser must be closed to invalidate basic authentication"
    )
        ...
        if action == "login":
            ...
            if check_credentials(username, password, request):
                ...
                if needs_password_upgrade():
                    ...
                    return {
                        "upgrade_hash": True,
                        "upgrade_token": session["_upgrade_token"],
                    }, 200
                return "", 204
            else:
