...
api_ns_system_languages = Namespace(
    "System Languages", description="Get languages list"
)
...
class Languages(Resource):
    ...
    get_request_parser.add_argument(
        "history", type=str, required=False, help="Language name for history stats"
    )
    ...
    def get(self):
        ...
        if history and history not in False_Keys:
            languages = database.execute(
                select(TableHistory.language).where(TableHistory.language.is_not(None))
            ).all()
            languages += database.execute(
                select(TableHistoryMovie.language).where(
                    TableHistoryMovie.language.is_not(None)
                )
            ).all()
