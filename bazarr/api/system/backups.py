...
api_ns_system_backups = Namespace(
    "System Backups", description="List, create, restore or delete backups"
)


@api_ns_system_backups.route("system/backups")
class SystemBackups(Resource):
    get_response_model = api_ns_system_backups.model(
        "SystemBackupsGetResponse",
        {
            "date": fields.String(),
    ...
    patch_request_parser.add_argument(
        "filename", type=str, required=True, help="Backups to restore filename"
    )
    ...
    delete_request_parser.add_argument(
        "filename", type=str, required=True, help="Backups to delete filename"
    )
