...
api_ns_system_searches = Namespace(
    "System Searches", description="Search for series or movies by name"
)
...
class Searches(Resource):
    ...
    get_request_parser.add_argument(
        "query", type=str, required=True, help="Series or movie name to search for"
    )
    ...
    def get(self):
        ...
        for x in search_list:
            if query in unidecode(x.title).lower():
                ...
                if hasattr(x, "sonarrSeriesId"):
                    ...
                    result["poster"] = (
                        f"{base_url}/images/series{x.poster}" if x.poster else None
                    )

                else:
                    result["radarrId"] = x.radarrId
                    result["poster"] = (
                        f"{base_url}/images/movies{x.poster}" if x.poster else None
                    )
        ...
        results = sorted(
            results, key=lambda x: textdistance.hamming.distance(query, x["title"])
        )
        return results
