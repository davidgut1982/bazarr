...
api_ns_system_settings = Namespace(
    "systemSettings", description="System settings API endpoint"
)
...
class SystemSettings(Resource):
    ...
    def get(self):
        ...
        data["notifications"]["providers"] = [
            {"name": x.name, "enabled": x.enabled == 1, "url": x.url}
    ...
    def post(self):
        ...
        if len(enabled_languages) != 0:
            database.execute(update(TableSettingsLanguages).values(enabled=0))
        ...
        if languages_profiles:
                ...
                if item["profileId"] in existing:
                    ...
                    database.execute(
                        ...
                        .values(
                            name=item["name"],
                            cutoff=item["cutoff"]
                            if item["cutoff"] not in None_Keys
                            else None,
                            items=json.dumps(item["items"]),
                            mustContain=str(item["mustContain"]),
                            mustNotContain=str(item["mustNotContain"]),
                            originalFormat=int(item["originalFormat"])
                            if item["originalFormat"] not in None_Keys
                            else None,
            ...
            for profileId in existing:
                ...
                database.execute(
                    delete(TableLanguagesProfiles).where(
        ...
        for item in notifications:
            ...
            database.execute(
                update(TableSettingsNotifier)
                .values(enabled=int(item["enabled"] is True), url=item["url"])
