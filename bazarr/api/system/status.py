...
api_ns_system_status = Namespace(
    "System Status", description="List environment information and versions"
)
    ...
    def get(self):
        ...
        try:
            database_version = ".".join(
                [str(x) for x in engine.dialect.server_version_info]
            )
        except Exception:
            database_version = ""

        try:
            database_migration = MigrationContext.configure(
                engine.connect()
        ...
        system_status.update(
            {
                "database_engine": f"{engine.dialect.name.capitalize()} {database_version}"
            }
        )
