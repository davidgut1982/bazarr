...
api_ns_series = Namespace(
    "Series",
    description="List series metadata, update series languages profile or run actions "
    "for specific series.",
)
...
class Series(Resource):
    ...
    get_subtitles_language_model = api_ns_series.model(
        "subtitles_language_model", subtitles_language_model
    )
    get_audio_language_model = api_ns_series.model(
        "audio_language_model", audio_language_model
    )

    data_model = api_ns_series.model(
        "series_data_model",
        {
    ...
    get_response_model = api_ns_series.model(
        "SeriesGetResponse",
        {
    ...
    def get(self):
        ...
        stmt = (
            ...
            .join(
                episodeMissingCount,
                TableShows.sonarrSeriesId == episodeMissingCount.c.sonarrSeriesId,
                isouter=True,
            )
        ...
        count = database.execute(select(func.count()).select_from(TableShows)).scalar()
        ...
        for idx in range(len(seriesIdList)):
            ...
            episode_id_list = database.execute(
                select(TableEpisodes.sonarrEpisodeId).where(
                    TableEpisodes.sonarrSeriesId == seriesId
                )
            ).all()
    ...
    patch_request_parser.add_argument(
        "seriesid", type=int, required=False, help="Sonarr series ID"
    )
    patch_request_parser.add_argument(
        "action",
        type=str,
