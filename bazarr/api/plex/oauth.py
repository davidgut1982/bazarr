...
def get_or_create_client_identifier():
    ...
    if not client_id:
        ...
        write_config()
        logger.info(
            f"Generated new persistent Plex client identifier: {client_id[:8]}..."
        )
    return client_id
...
def validate_plex_token(token):
    if not token:
        raise InvalidTokenError(  # noqa: F405
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            raise PlexConnectionError(  # noqa: F405
        ...
        raise PlexConnectionError(  # noqa: F405
            "Request to Plex.tv timed out. Please try again later."
        )
...
def refresh_token(token):
    ...
    try:
        ...
        if response.status_code == 401:
            raise TokenExpiredError(  # noqa: F405
        ...
        raise PlexConnectionError(  # noqa: F405
            "Token refresh request to Plex.tv timed out. Please try again later."
        )
    except requests.exceptions.RequestException as e:
...
def test_plex_connection(uri, token):
    ...
    try:
        ...
        response = requests.get(
            f"{uri}/identity", headers=headers, timeout=3, verify=get_ssl_verify("plex")
        )
        ...
        logger.debug(
            f"Plex connection test failed for {sanitize_log_data(uri)}: {type(e).__name__}"
        )
        return False, None
...
class PlexPin(Resource):
    ...
    post_request_parser.add_argument(
        "clientId", type=str, required=False, help="Client ID"
    )
...
class PlexPinCheck(Resource):
    ...
    def get(self, pin_id):
        try:
            ...
            if stored_state:
                if not state_param or not _validate_state_token(
                    state_param, stored_state
                ):
            ...
            response = requests.get(
                f"https://plex.tv/api/v2/pins/{pin_id}", headers=headers, timeout=10
            )
            ...
            return {"data": {"authenticated": False, "code": pin_data.get("code")}}
    ...
    def get(self):
        try:
            ...
            if not decrypted_token:
                return {"data": {"valid": False, "auth_method": auth_method}}, 200
            ...
            return {
                "data": {"valid": False, "error": e.message, "code": e.error_code}
            }, 200
    ...
    def get(self):
        try:
            ...
            if response.status_code in (401, 403):
                ...
                raise PlexConnectionError(  # noqa: F405
                    f"Failed to get servers: HTTP {response.status_code}"
                )
                ...
                if (
                    ...
                    if connection_candidates:
                        ...
                        def test_connection_wrapper(conn_data):
                            available, latency = test_plex_connection(
                                conn_data["uri"], decrypted_token
                            )
                            if available:
                        ...
                        with ThreadPoolExecutor(
                            max_workers=min(5, len(connection_candidates))
                        ) as executor:
                            future_to_conn = {
                    ...
                    if connections:
                        ...
                        if (
                            selected_machine_id
                            and device["clientIdentifier"] == selected_machine_id
                        ):
                            # Store ALL connection URIs (not just the working ones) for round-robin fallback
                            settings.plex.server_connections = (
                                all_device_connection_uris
                            )
                            # Update best connection if it changed
                            if bestConnection:
                                settings.plex.server_url = bestConnection["uri"]
                                settings.plex.server_local = bestConnection.get(
                                    "local", False
                                )
                            write_config()
                            logger.debug(
                                f"Auto-updated connections for server {device['name']}: {len(all_device_connection_uris)} total, {len(connections)} available"
                            )
            ...
            logger.warning(
                f"Unexpected error getting Plex servers: {type(e).__name__}: {str(e)}"
            )
            return {"data": []}
    ...
    def get(self):
        try:
            ...
            if primary_url:
                ...
                server_connections.extend(
                    [url for url in all_connections if url != primary_url]
                )
            ...
            logger.debug(
                f"Fetching Plex libraries for server: {settings.plex.get('server_name', 'Unknown')}"
            )
            ...
            for idx, server_url in enumerate(server_connections, 1):
                try:
                    logger.debug(
                        f"Attempting to fetch libraries from connection {idx}/{len(server_connections)}: {sanitize_server_url(server_url)}"
                    )
                    ...
                    if lib_response.status_code in (401, 403):
                        logger.debug(
                            f"Connection {idx}: Authentication failed ({lib_response.status_code})"
                        )
                        continue
                    elif lib_response.status_code != 200:
                        logger.debug(
                            f"Connection {idx}: HTTP {lib_response.status_code}"
                        )
                        continue
                    ...
                    if "application/json" in lib_content_type:
                        ...
                        if (
                            ...
                            and "Directory" in data["MediaContainer"]
                        ):
                            sections = data["MediaContainer"]["Directory"]
                    elif (
                        "application/xml" in lib_content_type
                        or "text/xml" in lib_content_type
                    ):
                        import xml.etree.ElementTree as ET
                    ...
                    if sections:
                        ...
                        logger.debug(
                            f"Successfully fetched libraries from connection {idx}/{len(server_connections)}"
                        )
                        break
                    else:
                    ...
                    logger.debug(
                        f"Connection {idx} failed: {type(e).__name__}: {str(e)}"
                    )
                    continue
                except Exception as e:
                    logger.debug(
                        f"Connection {idx} error: {type(e).__name__}: {str(e)}"
                    )
                    continue
            ...
            if not successful_server_url or not sections:
                logger.warning(
                    f"Failed to fetch libraries from all {len(server_connections)} connection(s)"
                )
                return {"data": []}
            ...
            for section in sections:
                if isinstance(section, dict) and section.get("type") in [
                    "movie",
                    "show",
                ]:
                    ...
                    try:
                        ...
                        if count_response.status_code == 200:
                            ...
                            if "MediaContainer" in count_data:
                                ...
                                actual_count = int(
                                    container.get(
                                        "size", len(container.get("Metadata", []))
                                    )
                                )

                        logger.debug(
                            f"Library '{section.get('title')}' has {actual_count} items"
                        )

                    except Exception as e:
                        logger.warning(
                            f"Failed to get count for library {section.get('title')}: {e}"
                        )
                        actual_count = 0
                    ...
                    libraries.append(
                        {
                            ...
                            "locations": _get_library_locations(
                                successful_server_url, section_key, decrypted_token
                            ),
                        }
                    )

            logger.debug(
                f"Successfully retrieved {len(libraries)} movie/show libraries from Plex"
            )
            return {"data": libraries}

        except requests.exceptions.RequestException as e:
            logger.warning(
                f"Failed to connect to Plex server: {type(e).__name__}: {str(e)}"
            )
            return {"data": []}
        except Exception as e:
            logger.warning(
                f"Unexpected error getting Plex libraries: {type(e).__name__}: {str(e)}"
            )
            return {"data": []}
...
class PlexSelectServer(Resource):
    ...
    post_request_parser = reqparse.RequestParser()
    post_request_parser.add_argument(
        "machineIdentifier", type=str, required=True, help="Machine identifier"
    )
    post_request_parser.add_argument(
        "name", type=str, required=True, help="Server name"
    )
    post_request_parser.add_argument(
        "uri", type=str, required=True, help="Connection URI"
    )
    post_request_parser.add_argument(
        "local", type=str, required=False, default="false", help="Is local connection"
    )
    post_request_parser.add_argument(
        "connections",
        type=list,
        location="json",
        required=False,
        help="All available connection URIs",
    )
    ...
    def post(self):
        ...
        settings.plex.server_connections = (
            connections if connections else [connection_uri]
        )
        write_config()
    ...
    def post(self):
        try:
            ...
            if not apikey:
                ...
                return {
                    "error": "No API key configured. Set up API key in Settings > General first."
                }, 400
            ...
            if not host:
                logger.error(
                    "Hostname must be configured in General Settings before creating webhooks. This prevents "
            ...
            if configured_base_url:
                ...
                logger.info(
                    f"Using configured base URL for webhook: {scheme}://{host}{configured_base_url}/api/webhooks/plex (instance: {instance_name})"
                )
            else:
                webhook_url = f"{scheme}://{host}/api/webhooks/plex?apikey={apikey}&instance={instance_param}"
                logger.info(
                    f"Using request host for webhook (no base URL configured): {scheme}://{host}/api/webhooks/plex (instance: {instance_name})"
                )
                logger.info(
                    "Note: If Bazarr is behind a reverse proxy, configure Base URL in General Settings for better reliability"
                )
            ...
            for webhook in webhooks:
                try:
                    ...
                    if hasattr(webhook, "url"):
                        ...
                        logger.warning(
                            f"Unknown webhook type: {type(webhook)}, value: {webhook}"
                        )
                        continue
            ...
            return {
                "data": {
                    ...
                    "plexPassSubscription": {
                        ...
                        "has_webhooks_feature": "webhooks"
                        in account.subscriptionFeatures,
...
class PlexWebhookDelete(Resource):
    post_request_parser = reqparse.RequestParser()
    post_request_parser.add_argument(
        "webhook_url", type=str, required=True, help="Webhook URL to delete"
    )
    ...
    def post(self):
        try:
            ...
            existing_webhooks = account.webhooks()
            logger.info(
                f"Existing webhooks before deletion: {[str(w) for w in existing_webhooks]}"
            )
            ...
            return {
                "data": {"success": True, "message": "Webhook deleted successfully"}
            }
