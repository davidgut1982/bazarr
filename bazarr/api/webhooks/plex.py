...
api_ns_webhooks_plex = Namespace(
    "Webhooks Plex",
    description="Webhooks endpoint that can be configured in Plex to "
...
def _is_relevant_server(payload):
    ...
    if not configured_server:
        ...
        logger.debug(
            f"PLEX WEBHOOK [{instance_name}]: No server configured, processing all"
        )
        return True
    ...
    if not server_uuid:
        ...
        logger.debug(
            f"PLEX WEBHOOK [{instance_name}]: No server UUID in payload, processing"
        )
        return True

    if server_uuid == configured_server:
        logger.debug(
            f"PLEX WEBHOOK [{instance_name}]: Server UUID matches ({server_uuid[:8]}...)"
        )
        return True

    logger.debug(
        f"PLEX WEBHOOK [{instance_name}]: Server UUID mismatch "
...
def _is_relevant_library(payload):
    ...
    if not configured_lib_ids and not configured_lib_names:
        logger.debug(
            f"PLEX WEBHOOK [{instance_name}]: No libraries configured, processing all"
        )
        return True
    ...
    if configured_lib_ids and library_section_id:
        if str(library_section_id) in [str(lid) for lid in configured_lib_ids]:
            logger.debug(
                f"PLEX WEBHOOK [{instance_name}]: Library ID {library_section_id} matches"
            )
            return True
    ...
    if configured_lib_names and library_section_title:
        if library_section_title in configured_lib_names:
            logger.debug(
                f'PLEX WEBHOOK [{instance_name}]: Library name "{library_section_title}" matches'
            )
            return True

    logger.debug(
        f'PLEX WEBHOOK [{instance_name}]: Library "{library_section_title}" '
...
class WebHooksPlex(Resource):
    ...
    post_request_parser.add_argument(
        "payload", type=str, required=True, help="Webhook payload"
    )
    ...
    def post(self):
        ...
        try:
            ...
            if "Metadata" not in parsed_json_webhook:
                logger.debug(
                    'PLEX WEBHOOK: No Metadata in payload for event "%s"', event
                )
                return "No Metadata found in JSON request body", 400

            if "Guid" not in parsed_json_webhook["Metadata"]:
                logger.debug(
                    'PLEX WEBHOOK: No GUID in Metadata for event "%s". Probably a pre-roll video.',
                    event,
                )
            ...
            logger.debug(
                "PLEX WEBHOOK: Failed to parse JSON. Error: %s. Payload: %s",
                str(e),
                sanitize_log_data(json_webhook) if json_webhook else "None",
            )
        ...
        if media_type == "episode":
            try:
                ...
                if not series_imdb_id:
                    episode_imdb_id = next(
                        (x["imdb"] for x in ids if "imdb" in x), None
                    )
                    if episode_imdb_id:
                        r = requests.get(
            ...
            try:
                ...
                radarrId = database.execute(
                    select(TableMovies.radarrId).where(
                        TableMovies.imdbId == movie_imdb_id
                    )
                ).first()
