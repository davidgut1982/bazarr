...
api_ns_episodes_subtitles = Namespace(
    "Episodes Subtitles", description="Download, upload or delete episodes subtitles"
)
...
class EpisodesSubtitles(Resource):
    ...
    patch_request_parser.add_argument(
        "forced", type=str, required=True, help="Forced true/false as string"
    )
    patch_request_parser.add_argument(
        "hi", type=str, required=True, help="HI true/false as string"
    )
    ...
    @api_ns_episodes_subtitles.response(
    ...
    def patch(self):
        ...
        episode_download_specific_subtitles(
    ...
    def post(self):
        ...
        episodeInfo = database.execute(
            select(TableEpisodes.path, TableEpisodes.audio_language).where(
                TableEpisodes.sonarrEpisodeId == sonarrEpisodeId
            )
        ).first()
    ...
    delete_request_parser.add_argument(
        "path", type=str, required=True, help="Path of the subtitles file"
    )
