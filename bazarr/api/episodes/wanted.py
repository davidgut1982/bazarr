...
api_ns_episodes_wanted = Namespace(
    "Episodes Wanted", description="List episodes wanted subtitles"
)
...
class EpisodesWanted(Resource):
    ...
    get_subtitles_language_model = api_ns_episodes_wanted.model(
        "subtitles_language_model", subtitles_language_model
    )
    get_audio_language_model = api_ns_episodes_wanted.model(
        "audio_language_model", audio_language_model
    )

    data_model = api_ns_episodes_wanted.model(
        "wanted_episodes_data_model",
        {
    ...
    get_response_model = api_ns_episodes_wanted.model(
        "EpisodeWantedGetResponse",
        {
    ...
    def get(self):
        ...
        if length > 0:
            stmt = (
                stmt.order_by(TableEpisodes.sonarrEpisodeId.desc())
                .limit(length)
                .offset(start)
            )
