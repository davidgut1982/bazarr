...
api_ns_history_stats = Namespace(
    "History Statistics", description="Get history statistics"
)
...
class HistoryStats(Resource):
    ...
    get_request_parser.add_argument(
        "action", type=str, default="All", help="Action type to filter for."
    )
    get_request_parser.add_argument(
        "provider", type=str, default="All", help="Provider name to filter for."
    )
    get_request_parser.add_argument(
        "language", type=str, default="All", help="Language name to filter for"
    )

    series_data_model = api_ns_history_stats.model(
        "history_series_stats_data_model",
        {
            "date": fields.String(),
    ...
    movies_data_model = api_ns_history_stats.model(
        "history_movies_stats_data_model",
        {
            "date": fields.String(),
            "count": fields.Integer(),
        },
    ...
    get_response_model = api_ns_history_stats.model(
        "HistoryStatsGetResponse",
        {
    ...
    def get(self):
        ...
        data_series = [
            ...
            for x in database.execute(
                select(TableHistory.timestamp, TableHistory.id).where(
                    history_where_clause
                )
            ).all()
        ]
        data_series = [
            {"date": date[0], "count": sum(1 for item in date[1])}
            for date in itertools.groupby(
                list(data_series), key=lambda x: x["timestamp"].strftime("%Y-%m-%d")
            )
        ]
        ...
        data_movies = [
            ...
            for x in database.execute(
                select(TableHistoryMovie.timestamp, TableHistoryMovie.id).where(
                    history_where_clause_movie
                )
            ).all()
        ]
        data_movies = [
            {"date": date[0], "count": sum(1 for item in date[1])}
            for date in itertools.groupby(
                list(data_movies), key=lambda x: x["timestamp"].strftime("%Y-%m-%d")
            )
        ]
        ...
        return marshal(
            {"series": sorted_data_series, "movies": sorted_data_movies},
