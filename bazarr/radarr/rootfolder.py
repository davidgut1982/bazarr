    ...
    try:
        ...
        for folder in rootfolder.json():
            if any(
                ...
                for item in database.execute(select(TableMovies.path)).all()
            ):
        ...
        for item in rootfolder_to_remove:
            database.execute(
                delete(TableMoviesRootfolder).where(TableMoviesRootfolder.id == item.id)
            )
        ...
        for item in rootfolder_to_insert:
            database.execute(
                insert(TableMoviesRootfolder).values(id=item["id"], path=item["path"])
            )
    ...
    for item in rootfolder:
        ...
        if not os.path.isdir(path_mappings.path_replace_movie(root_path)):
            database.execute(
                ...
                .values(
                    accessible=0,
                    error="This Radarr root directory does not seem to be accessible by Bazarr. "
            ...
            if os.access(mapped_path, os.W_OK):
                ...
                try:
                    ...
                    database.execute(
                        ...
                        .values(
                            accessible=0,
                            error=f"There's an issue with this Radarr root directory: {repr(e)}",
                        )
                        .where(TableMoviesRootfolder.id == item.id)
                    )
                else:
