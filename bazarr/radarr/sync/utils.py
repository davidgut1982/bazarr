...
def get_profile_list():
    ...
    try:
        profiles_json = radarr_session().get(
        ...
        if get_radarr_info.is_legacy():
            for profile in profiles_json.json():
                if "language" in profile:
                    profiles_list.append(  # noqa: PERF401
...
def get_tags():
    ...
    try:
        tagsDict = radarr_session().get(
            url_radarr_api_series,
            timeout=int(settings.radarr.http_timeout),
            verify=get_ssl_verify("radarr"),
            headers=radarr_headers(apikey_radarr),
        )
...
def get_movies_from_radarr_api(apikey_radarr, radarr_id=None):
    url_radarr_api_movies = (
        f"{url_api_radarr()}movie{f'/{radarr_id}' if radarr_id else ''}"
    )

    try:
        r = radarr_session().get(
            url_radarr_api_movies,
            timeout=int(settings.radarr.http_timeout),
            verify=get_ssl_verify("radarr"),
            headers=radarr_headers(apikey_radarr),
...
def get_history_from_radarr_api(apikey_radarr, movie_id):
    url_radarr_api_history = (
        f"{url_api_radarr()}history?eventType=1&movieIds={movie_id}"
    )
    ...
    try:
        ...
        logging.exception(
            "BAZARR Error trying to get history from Radarr. Connection Error."
        )
        return
    except requests.exceptions.Timeout:
        logging.exception(
            "BAZARR Error trying to get history from Radarr. Timeout Error."
        )
        return
    except requests.exceptions.RequestException:
        ...
        logging.exception(
            f"Exception raised while getting history from Radarr API: {e}"  # noqa: G004
        )
        return
    else:
        if r.status_code == 200:
