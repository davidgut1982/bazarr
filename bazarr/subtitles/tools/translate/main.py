...
def translate_subtitles_file(
    ...
    if not job_id:
        ...
        jobs_queue.add_job_from_function(
            (
                lambda t: f"Translating {t} ({from_lang.upper()} to {to_lang.upper()})"
                if t
                else f"Translating {from_lang.upper()} to {to_lang.upper()}"
    ...
    try:
        logging.debug(
            f"Translation request: video={video_path}, source={source_srt_file}, from={from_lang}, to={to_lang}"  # noqa: G004
        )
        ...
        logging.debug(
            f"BAZARR is translating in {lang_obj} this subtitles {source_srt_file}"  # noqa: G004
        )
        ...
        dest_srt_file = get_external_subtitles_path(
            file=video_path, subtitle=os.path.basename(dest_srt_file_if_alongside_video)
        )
