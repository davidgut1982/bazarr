...
class LingarrTranslatorService:
    ...
    def translate(self, job_id=None):
        try:
            jobs_queue.update_job_progress(
                job_id=job_id, progress_max=1, progress_message=self.source_srt_file
            )
            ...
            logger.debug(
                f"BAZARR saving Lingarr translated subtitles to {self.dest_srt_file}"  # noqa: G004
            )
            translation_map = {}
            ...
            logger.error(
                f"BAZARR encountered an error during Lingarr translation: {str(e)}"  # noqa: G004
            )
            jobs_queue.update_job_progress(
                job_id=job_id, progress_message=f"Lingarr translation failed: {str(e)}"
            )
            raise
    ...
    def _translate_content(self, lines_list, job_id):
        try:
            ...
            logger.debug(
                f"BAZARR is sending {len(lines_payload)} lines to Lingarr with full media context"  # noqa: G004
            )
            ...
            if response.status_code == 200:
                ...
                if isinstance(translated_batch, list):
                    for item in translated_batch:
                        if (
                            ...
                            logger.error(
                                f"Invalid response format from Lingarr API: {item}"  # noqa: G004
                            )
                            return None
                    return translated_batch
                else:
                    logger.error(
                        f"Unexpected response format from Lingarr API: {translated_batch}"  # noqa: G004
                    )
                    return None
            elif response.status_code == 401:
                raise LingarrAuthError(
                ...
                logger.debug(
                    f"Lingarr API error: {response.status_code} - {response.text}"  # noqa: G004
                )
                return None
            ...
            jobs_queue.update_job_progress(
                job_id=job_id, progress_message=f"Lingarr API error: {str(e)}"
            )
            raise
        except Exception as e:
            logger.error(f"Unexpected error in Lingarr translation: {str(e)}")  # noqa: G004
            jobs_queue.update_job_progress(
                job_id=job_id, progress_message=f"Translation error: {str(e)}"
            )
            raise
