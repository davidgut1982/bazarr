        ...
        try:
            ...
            def translate_line(line_id, subtitle_line):
                try:
                    ...
                    jobs_queue.update_job_progress(
                        job_id=job_id, progress_value=len(translated_lines)
                    )

            logger.debug(
                f"BAZARR is sending {lines_list_len} blocks to Google Translate"  # noqa: G004
            )
            ...
            for i, line in enumerate(subs):
                try:
                    ...
                    logger.error(
                        f"BAZARR is unable to translate malformed subtitles: {self.source_srt_file}"  # noqa: G004
                    )
                    jobs_queue.update_job_progress(
                        job_id=job_id,
            ...
            jobs_queue.update_job_progress(
                job_id=job_id, progress_message=f"Google translation failed: {str(e)}"
            )
            raise

    @retry(
        exceptions=(TooManyRequests, RequestError),
        tries=6,
        delay=1,
        backoff=2,
        jitter=(0, 1),
    ...
    def _translate_text(self, text, job_id):
        try:
            return GoogleTranslator(
                source="auto",
                target=self.language_code_convert_dict.get(
                    self.lang_obj.alpha2, self.lang_obj.alpha2
                ),
            ).translate(text=text)
        except (TooManyRequests, RequestError) as e:
            logger.error(f"Google Translate API error after retries: {str(e)}")  # noqa: G004
            jobs_queue.update_job_progress(
                job_id=job_id, progress_message=f"Google Translate API error: {str(e)}"
            )
            raise
        except Exception as e:
