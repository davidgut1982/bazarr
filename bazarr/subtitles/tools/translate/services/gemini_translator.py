...
class GeminiTranslatorService:
    ...
    def translate(self, job_id):
        ...
        try:
            ...
            logger.info(
                "BAZARR is sending subtitle file to Gemini for translation "  # noqa: G003
                + self.source_srt_file
            )
            ...
            self.description = get_description(
                self.media_type, self.radarr_id, self.sonarr_series_id
            )
            ...
            logger.error(
                f"BAZARR encountered an error translating with Gemini: {str(e)}"  # noqa: G004
            )
            raise
    ...
    def _check_saved_progress(self):
        ...
        try:
            ...
            jobs_queue.update_job_progress(
                job_id=self.job_id, progress_message=f"Error reading progress file: {e}"
            )
    ...
    def _save_progress(self, line):
        ...
        try:
            ...
            jobs_queue.update_job_progress(
                job_id=self.job_id, progress_message=f"Failed to save progress: {e}"
            )
    ...
    def _handle_rate_limited_key(self, response):
        if not self.current_api_key:
            raise RuntimeError(
                "All Gemini API keys are currently rate limited. Please wait before retrying."
            )
                ...
                "parts": [
                    {
                        "text": self.get_instruction(
                            self.target_language, self.description
                        )
                    }
                ]
            },
    ...
    def _process_batch(
        ...
        try:
            ...
            jobs_queue.update_job_progress(
                job_id=self.job_id, progress_value=self.current_progress
            )
            ...
            if self._is_rate_limited_response(response):
                ...
                if retry_num > 0:
                    return self._process_batch(
                        batch, translated_subtitle, total, retry_num - 1
                    )
            ...
            if retry_num > 0:
                ...
                jobs_queue.update_job_progress(
                    job_id=self.job_id,
                    progress_message=f"Translation request failed: {e}",
                )
                raise e
    ...
    def _translate_with_gemini(self):
        ...
        if not self.target_language:
            jobs_queue.update_job_progress(
                job_id=self.job_id, progress_message="Please provide a target language."
            )
            return

        if not self.input_file:
            jobs_queue.update_job_progress(
                job_id=self.job_id, progress_message="Please provide a subtitle file."
            )
            return
        ...
        try:
            with open(self.input_file, "r", encoding="utf-8") as original_file:
                ...
                with open(self.output_file, "w", encoding="utf-8") as translated_file:
                    ...
                    batch = [
                        SubtitleObject(
                            index=str(i), content=original_subtitle[i].content
                        )
                    ]
                    i += 1
                    ...
                    while (i < total or len(batch) > 0) and not self.interrupt_flag:
                        ...
                        try:
                            ...
                            if (
                                delay
                                and (end_time - start_time < delay_time)
                                and i < total
                            ):
