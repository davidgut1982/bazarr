...
def process_episode_translation(
    ...
    if not source_subtitle_path:
        logger.error(
            f"No subtitle found for episode {sonarr_episode_id} (requested source: {source_language})"  # noqa: G004
        )
        return False
...
def process_movie_translation(
    ...
    if not source_subtitle_path:
        logger.error(
            f"No subtitle found for movie {radarr_id} (requested source: {source_language})"  # noqa: G004
        )
        return False
...
def extract_embedded_subtitle(video_path, language_code2, media_type):
    ...
    if found_track is None:
        logger.debug(
            f"No extractable embedded subtitle found for language {language_code2} in {video_path}"  # noqa: G004
        )
        return None
    ...
    try:
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            ...
            logger.info(
                f"Extracted embedded {language_code2} subtitle to: {output_path}"  # noqa: G004
            )
            return output_path
        return None
