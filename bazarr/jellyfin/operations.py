...
def _redact(text, override_secret: str = None) -> str:
    ...
    try:
        ...
        pass
    if (
        isinstance(override_secret, str)
        and override_secret
        and override_secret != saved
    ):
...
def get_jellyfin_client(
    url: str = None, apikey: str = None, verify_ssl: bool = None
) -> JellyfinClient:
...
def jellyfin_test_connection(
    ...
    try:
        ...
        logger.error(
            f"Failed to connect to Jellyfin server: {_redact(e, override_secret=apikey)}"  # noqa: G004
        )
        return {
            "success": False,
...
def jellyfin_get_libraries(
    ...
    try:
        ...
        logger.debug(
            f"Jellyfin returned {len(libraries)} library folders: "  # noqa: G004
        ...
        logger.error(
            f"Failed to get Jellyfin libraries: {_redact(e, override_secret=apikey)}"  # noqa: G004
        )
        ...
        try:
            ...
            for item in items:
                ...
                if (
                    title
                    and not title_match
                    and item.get("Name", "").lower() == title.lower()
                ):
                    title_match = item
...
def _find_episode(
    client: JellyfinClient, series_id: str, season: int, episode: int
) -> str | None:
    ...
    try:
        ...
        logger.debug(
            f"Error finding episode S{season:02d}E{episode:02d} in series {series_id}: {_redact(e)}"  # noqa: G004
        )

    return None


def jellyfin_refresh_item(
    imdb_id: str = None,
    is_movie: bool = True,
    season: int = None,
    episode: int = None,
    ...
    try:
        ...
        if is_movie:
            ...
            if item:
                _refresh_or_report(
                    client, immediate, item_id=item["Id"], path=item.get("Path")
                )
                logger.info(
                    f"Refreshed movie in Jellyfin: {item.get('Path', item['Id'])}"  # noqa: G004
                )
                return
        else:
            series = _find_item(
            ...
            if series and season is not None and episode is not None:
                if immediate:
                    ...
                    if episode_id:
                        ...
                        logger.info(
                            f"Refreshed episode in Jellyfin (immediate): "  # noqa: G004
                    ...
                    _refresh_or_report(
                        client, immediate, item_id=series["Id"], path=series.get("Path")
                    )
                    logger.info(
                        f"Reported series update to Jellyfin (async): "  # noqa: G004
        ...
        logger.warning(
            f"Item not found in Jellyfin (IMDB: {imdb_id}, TMDB: {tmdb_id}, TVDB: {tvdb_id}), "  # noqa: G004
        ...
        logger.warning(
            f"Failed to refresh Jellyfin item, falling back to library update: {_redact(e)}"  # noqa: G004
        )
        try:
...
def _refresh_or_report(
    client: JellyfinClient, immediate: bool, item_id: str = None, path: str = None
) -> None:
...
def jellyfin_update_library(
    client: JellyfinClient = None,
    is_movie_library: bool = True,
    ...
    try:
        ...
        for library_id in library_ids:
            ...
            try:
                ...
                logger.error(
                    f"Failed to refresh Jellyfin library '{library_id}': {_redact(e)}"  # noqa: G004
                )
                continue

        if updated_count > 0:
            logger.debug(
                f"Successfully triggered refresh for {updated_count} Jellyfin libraries"  # noqa: G004
            )
        else:
