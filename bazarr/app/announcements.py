...
def parse_announcement_dict(announcement_dict):
    ...
    announcement_dict["hash"] = hashlib.sha256(
        announcement_dict["text"].encode("UTF8")
    ).hexdigest()
...
def get_announcements_to_file(job_id=None, startup=False, wait_for_completion=False):
    ...
    try:
        ...
        try:
            logging.exception(
                "Error trying to get announcements from jsdelivr.net, falling back to Github."
            )
            r = _announcements_session().get(
    ...
    if not startup:
        jobs_queue.update_job_name(
            job_id=job_id, new_job_name="Updated Announcements File"
        )
...
def get_online_announcements():
    try:
        with open(
            os.path.join(args.config_dir, "config", "announcements.json"),
            "r",
            encoding="utf-8",
        ) as f:
            data = json.load(f)
...
def get_all_announcements():
    ...
    announcements = [
        parse_announcement_dict(x)
        for x in get_online_announcements() + get_local_announcements()
        if x["enabled"]
        and (
            not x["dismissible"]
            or not database.execute(
                select(TableAnnouncements).where(
                    TableAnnouncements.hash
...
def mark_announcement_as_dismissed(hashed_announcement):
    text = [
        x["text"] for x in get_all_announcements() if x["hash"] == hashed_announcement
    ]
    if text:
        database.execute(
            insert(TableAnnouncements)
            .values(hash=hashed_announcement, timestamp=datetime.now(), text=text[0])
