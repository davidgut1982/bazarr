...
while failed_validator:
    try:
        ...
        if (
            ...
            logging.warning(
                f"VALIDATOR RESET: {current_validator_details.names[0]} from '{old_value}' to '{current_validator_details.default}'"  # noqa: G004
            )
        else:
            logging.critical(
...
def write_config():
    ...
    try:
        ...
        logging.exception(
            f"Exception raised while trying to save temporary settings file: {error}"  # noqa: G004
        )
    else:
        try:
...
def migrate_apikey_to_oauth():
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if user_response.status_code == 429:  # Rate limited
                    logging.warning(
                        f"Rate limited by Plex API, attempt {attempt + 1}/{max_retries}"  # noqa: G004
                    )
                ...
                logging.warning(
                    f"Timeout getting user data, attempt {attempt + 1}/{max_retries}"  # noqa: G004
                )
                if attempt < max_retries - 1:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if servers_response.status_code == 429:  # Rate limited
                    logging.warning(
                        f"Rate limited getting servers, attempt {attempt + 1}/{max_retries}"  # noqa: G004
                    )
                    if attempt < max_retries - 1:
                ...
                logging.warning(
                    f"Timeout getting servers, attempt {attempt + 1}/{max_retries}"  # noqa: G004
                )
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
        ...
        logging.info(
            f"Selected server: {selected_server['name']} ({selected_connection['uri']})"  # noqa: G004
        )
...
def migrate_plex_library_to_list():
    ...
    if isinstance(settings.plex.movie_library, str):
        ...
        if old_value:  # Only migrate if not empty
            ...
            logging.info(
                f"Migrated plex.movie_library from string to list: {old_value}"  # noqa: G004
            )
            changed = True
        else:
    ...
    if isinstance(settings.plex.series_library, str):
        ...
        if old_value:  # Only migrate if not empty
            ...
            logging.info(
                f"Migrated plex.series_library from string to list: {old_value}"  # noqa: G004
            )
            changed = True
        else:
