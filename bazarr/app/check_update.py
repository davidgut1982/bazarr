...
def deprecated_python_version():
    ...
    return sys.version_info.major == 2 or (
...
def _fetch_repo_releases(repo, label=None):
    ...
    try:
        ...
        logging.exception(
            f"Error trying to get releases from Github ({repo}). Http error."  # noqa: G004
        )
    except requests.exceptions.ConnectionError:
        logging.exception(
            f"Error trying to get releases from Github ({repo}). Connection Error."  # noqa: G004
        )
    except requests.exceptions.Timeout:
        logging.exception(
            f"Error trying to get releases from Github ({repo}). Timeout Error."  # noqa: G004
        )
        ...
        try:
            ...
            logging.error(
                f"Error parsing JSON from Github releases response ({repo}). Skipping."  # noqa: G004
            )
            return releases
...
def check_if_new_update():
    ...
    if settings.general.branch == "master":
        ...
        logging.error(
            f"BAZARR unknown branch provided to updater: {settings.general.branch}"  # noqa: G004
        )
        return
    ...
    if not args.no_update:
            ...
            if deprecated_python_version():
                release = next(
                    (
                        item["name"].lstrip("v")
                        for item in data
                        if semver.VersionInfo.parse("1.3.1")
                        > semver.VersionInfo.parse(item["name"].lstrip("v"))
                    )
                )
            else:
            ...
            if deprecated_python_version():
                next(
                    (
                        ...
                        and semver.VersionInfo.parse("1.3.1")
                        > semver.VersionInfo.parse(item["name"].lstrip("v"))
                    )
                )
            else:
        ...
        if release and "name" in release:
            ...
            if deprecated_python_version():
                logging.warning(
                    "BAZARR is using a deprecated Python version, you must update Python to get latest "
                    "version available."
                )
            ...
            if new_version and release["name"] != "v0.9.1.1":
                logging.debug(
                    f"BAZARR newer release available and will be downloaded: {release['name']}"  # noqa: G004
                )
            ...
            # rolling back from nightly to stable release
                ...
                if current_version.prerelease and not use_prerelease:
                    logging.debug(
                        f"BAZARR previous stable version will be downloaded: {release['name']}"  # noqa: G004
                    )
                    download_release(url=release["download_link"])
            else:
...
def apply_update():
    ...
    if os.path.isdir(update_dir):
        if os.path.isfile(bazarr_zip):
            logging.debug(
                f"BAZARR is trying to unzip this release to {bazarr_dir}: {bazarr_zip}"  # noqa: G004
            )
            try:
...
def update_cleaner(zipfile, bazarr_dir, config_dir):
    ...
    logging.debug(
        f"BAZARR upgrade leftover cleaner will ignore directories matching this "  # noqa: G004
    ...
    logging.debug(
        f"BAZARR upgrade leftover cleaner will ignore those files: {', '.join(file_to_ignore)}"  # noqa: G004
    )
