...
class Scheduler:
    def __init__(self):
        ...
        try:
            ...
            logging.error(
                "BAZARR cannot use the specified timezone and will use UTC instead."
            )
        ...
        self.aps_scheduler = BackgroundScheduler(
            {"apscheduler.timezone": self.timezone}
        )
        ...
        self.aps_scheduler.add_listener(
            _release_session_after_job, EVENT_JOB_EXECUTED | EVENT_JOB_ERROR
        )
    ...
    def get_task_list(self):
        ...
        for job in self.aps_scheduler.get_jobs():
            ...
            if isinstance(job.trigger, IntervalTrigger):
                interval = get_time_from_interval(
    ...
    def __sonarr_update_task(self):
        if settings.general.use_sonarr:
            self.aps_scheduler.add_job(
                update_series,
                "interval",
                minutes=int(settings.sonarr.series_sync),
                max_instances=1,
                coalesce=True,
    ...
    def __radarr_update_task(self):
        if settings.general.use_radarr:
            self.aps_scheduler.add_job(
                update_movies,
                "interval",
                minutes=int(settings.radarr.movies_sync),
                max_instances=1,
                coalesce=True,
                misfire_grace_time=15,
                id="update_movies",
        ...
        if settings.general.use_radarr:
            ...
            if full_update == "Daily":
                self.aps_scheduler.add_job(
                    ...
                    misfire_grace_time=15,
                    kwargs=dict(wait_for_completion=True),
                    id="movies_full_scan_subtitles",
                    name="Index All Existing Movies Subtitles",
                    replace_existing=True,
                )
            elif full_update == "Weekly":
                self.aps_scheduler.add_job(
    ...
    def __update_bazarr_task(self):
        ...
        self.aps_scheduler.add_job(
            ...
            name="Update Announcements File",
            replace_existing=True,
            kwargs=dict(wait_for_completion=True),
        )
    ...
    def __upgrade_subtitles_task(self):
        if settings.general.use_sonarr or settings.general.use_radarr:
            self.aps_scheduler.add_job(
                upgrade_subtitles,
                "interval",
                hours=int(settings.general.upgrade_frequency),
                max_instances=1,
                coalesce=True,
                misfire_grace_time=15,
    ...
    def __mass_sync_task(self):
        self.aps_scheduler.add_job(
            ...
            coalesce=True,
            misfire_grace_time=15,
            id="mass_sync_subtitles",
            name="Mass Sync All Subtitles",
            replace_existing=True,
        )

    def __provider_hub_update_task(self):
        self.aps_scheduler.add_job(
            provider_hub_check_updates,
            "interval",
            hours=6,
            max_instances=1,
            coalesce=True,
            misfire_grace_time=15,
            id="provider_hub_update_check",
            name="Check Provider Hub Updates",
            replace_existing=True,
            kwargs=dict(wait_for_completion=True),
        )
