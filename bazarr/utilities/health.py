...
def get_health_issues():
    ...
    if settings.general.use_sonarr:
        ...
        for item in rootfolder:
            health_issues.append(  # noqa: PERF401
                {
                    "object": path_mappings.path_replace(item.path),
                    "issue": item.error,
                }
            )
    ...
    languages_profiles_count = database.execute(
        select(func.count(TableLanguagesProfiles.profileId))
    ).scalar()
    series_with_profile = database.execute(
        select(func.count(TableShows.sonarrSeriesId)).where(
            TableShows.profileId.is_not(None)
        )
    ).scalar()
    movies_with_profile = database.execute(
        select(func.count(TableMovies.radarrId)).where(
            TableMovies.profileId.is_not(None)
        )
    ).scalar()
    default_series_profile_empty = (
        settings.general.serie_default_enabled
        and settings.general.serie_default_profile == ""
    )
    default_movies_profile_empty = (
        settings.general.movie_default_enabled
        and settings.general.movie_default_profile == ""
    )
    ...
    elif languages_profiles_count > 0 and (
        (
            settings.general.use_sonarr
            and series_with_profile == 0
            and default_series_profile_empty
        )
        or (
            settings.general.use_radarr
            and movies_with_profile == 0
            and default_movies_profile_empty
        )
    ):
        health_issues.append(
            {
                "object": "No assigned languages profile",
