...
def generate_autopulse_config(decrypted_token=None):
    ...
    try:
        ...
        if not decrypted_token or not server_url:
            logging.warning(
                "BAZARR missing required Plex OAuth configuration for Autopulse"
            )
            return None
        ...
        if not autopulse_template:
            logging.error(
                "BAZARR Autopulse template API unavailable - please ensure Autopulse is running and supports the template API"
            )
            return None
        ...
        return _generate_config_from_template(
            autopulse_template, server_url, decrypted_token, server_name
        )
...
def _get_autopulse_template():
    ...
    try:
        ...
        if not autopulse_url:
            logging.debug(
                "BAZARR no Autopulse URL configured, skipping template request"
            )
            return None
        ...
        response = _make_autopulse_request(
            autopulse_url, params=params, auth=auth, headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            logging.warning(
                "BAZARR Autopulse template API authentication failed (401). Check your credentials. Did you forget to save your settings?"
            )
            return None
        elif response.status_code == 400:
            logging.warning(
                "BAZARR Autopulse template API bad request (400). Check your webhook URL and credentials. Did you forget to save your settings?"
            )
            return None
        else:
            logging.warning(
                f"BAZARR Autopulse template API failed with status {response.status_code}"  # noqa: G004
            )
            return None

    except requests.exceptions.RequestException as e:
        logging.debug(
            f"BAZARR Autopulse template API request failed: {str(e)}. Did you forget to save your external webhook settings?"  # noqa: G004
        )
        return None
    except Exception as e:
        logging.error(
            f"BAZARR unexpected error calling Autopulse template API: {str(e)}"  # noqa: G004
        )
        return None
...
def _generate_config_from_template(
    template_data, server_url, decrypted_token, server_name
):
    """
    ...
    try:
        ...
        config_with_values = config_with_values.replace(
            "refresh = false", "refresh = true"
        )
        config_with_values = config_with_values.replace(
            "analyze = true", "analyze = false"
        )
        ...
        if rewrite_config["detected"]:
            config_with_values = _inject_rewrite_config(
                config_with_values, rewrite_config
            )
        ...
        rewrite_status = (
            ...
            if rewrite_config["detected"]
            else "Path rewriting: Not detected"
        )
...
"""
        ...
        template_info = (
            f"Configuration generated using Autopulse template v{template_version}"
        )
        path_suggestion = (
            rewrite_config["suggestion"] if rewrite_config["suggestion"] else None
        )
...
def call_external_webhook(subtitle_path, media_path, language, media_type):
    ...
    try:
        ...
        logging.debug(
            f"BAZARR calling external webhook: {webhook_url} for path: {parent_dir}"  # noqa: G004
        )
        ...
        if response.status_code == 200:
            ...
            logging.warning(
                f"BAZARR external webhook authentication failed (401) for {parent_dir}. Did you forget to save your external webhook settings?"  # noqa: G004
            )
        elif response.status_code == 400:
            logging.warning(
                f"BAZARR external webhook bad request (400) for {parent_dir}. Check your webhook URL and credentials. Did you forget to save your external webhook settings?"  # noqa: G004
            )
        else:
            logging.warning(
                f"BAZARR external webhook failed with status {response.status_code} for {parent_dir}"  # noqa: G004
            )

    except requests.exceptions.RequestException as e:
        logging.error(
            f"BAZARR external webhook failed for {media_path}: {str(e)}. Did you forget to save your external webhook settings?"  # noqa: G004
        )
    except Exception as e:
        logging.error(
            f"BAZARR unexpected error calling external webhook for {media_path}: {str(e)}"  # noqa: G004
        )
...
def _make_webhook_request(url, auth, headers):
    ...
    return requests.get(url, auth=auth, headers=headers, timeout=30, verify=True)
...
def _make_autopulse_request(url, params=None, auth=None, headers=None):
    ...
    return requests.get(
        url, params=params, auth=auth, headers=headers, timeout=10, verify=True
    )
...
def test_external_webhook_connection():
    ...
    try:
        ...
        if not webhook_url:
            ...
            return (
                False,
                "External webhook not configured. Please enter a webhook URL and save your settings.",
            )
        ...
        response = requests.get(
            test_url, auth=auth, headers=headers, timeout=10, verify=True
        )
        ...
        if response.status_code == 200:
            ...
            return (
                False,
                "External webhook authentication failed (401). Check your username and password.",
            )
        elif response.status_code == 400:
            return (
                False,
                "External webhook bad request (400). Check your webhook URL and credentials.",
            )
        else:
            return (
                False,
                f"External webhook connection failed with status {response.status_code}. Check your webhook configuration.",
            )
        ...
        if "Connection refused" in error_msg:
            return (
                False,
                "External webhook connection refused. If using 'localhost', try using the Docker container name instead.",
            )
        elif (
            ...
            return (
                False,
                "External webhook hostname not found. Check your webhook URL for typos or incorrect container name.",
            )
        else:
        ...
        return (
            False,
            "External webhook connection timed out. Check if the service is running and accessible.",
        )
    except requests.exceptions.RequestException as e:
    ...
    if settings.general.use_external_webhook:
        ...
        if webhook_url:
            ...
            if not webhook_url.startswith(("http://", "https://")):
                logging.warning(
                    f"BAZARR invalid webhook URL format: {webhook_url} (must start with http:// or https://)"  # noqa: G004
                )
                return None
            return webhook_url
...
def _compare_path_sets(bazarr_path, plex_paths, media_type):
    ...
    for plex_path in plex_paths:
        ...
        rewrite_result = _detect_mount_point_difference(
            bazarr_normalized, plex_normalized, media_type
        )
...
def _detect_mount_point_difference(bazarr_path, plex_path, media_type):
    ...
    if (
        len(bazarr_parts) > len(plex_parts)
        and bazarr_parts[-len(plex_parts) :] == plex_parts[-len(plex_parts) :]
    ):
    ...
    # Check if Plex has extra prefix (e.g., /data/media vs /media)
        ...
        len(plex_parts) > len(bazarr_parts)
        and plex_parts[-len(bazarr_parts) :] == bazarr_parts[-len(bazarr_parts) :]
    ):
...
def _detect_common_patterns():
    ...
    for bazarr_pattern, plex_pattern in common_rewrites:
        if (movie_path and bazarr_pattern in movie_path) or (
            series_path and bazarr_pattern in series_path
        ):
            return {
                "detected": True,
    ...
    for line in lines:
        ...
        if "[triggers.bazarr]" in line:
            ...
            if rewrite_config["trigger_section"]:
                modified_lines.extend(
                    rewrite_config["trigger_section"].strip().split("\n")
                )
            in_bazarr_trigger = False
        ...
        if "[targets.plex]" in line:
            ...
            if rewrite_config["target_section"]:
                modified_lines.extend(
                    rewrite_config["target_section"].strip().split("\n")
                )
            in_plex_target = False
