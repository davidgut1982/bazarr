...
def pp_replace(
    ...
    pp_command = _pp_sub(
        r'[\'"]?{{directory}}[\'"]?', os.path.dirname(episode), pp_command
    )
    pp_command = _pp_sub(r'[\'"]?{{episode}}[\'"]?', episode, pp_command)
    pp_command = _pp_sub(
        r'[\'"]?{{episode_name}}[\'"]?',
        os.path.splitext(os.path.basename(episode))[0],
        pp_command,
    )
    pp_command = _pp_sub(r'[\'"]?{{subtitles}}[\'"]?', str(subtitles), pp_command)
    pp_command = _pp_sub(
        r'[\'"]?{{subtitles_language}}[\'"]?', str(language), pp_command
    )
    pp_command = _pp_sub(
        r'[\'"]?{{subtitles_language_code2}}[\'"]?', str(language_code2), pp_command
    )
    pp_command = _pp_sub(
        r'[\'"]?{{subtitles_language_code3}}[\'"]?', str(language_code3), pp_command
    )
    pp_command = _pp_sub(
        r'[\'"]?{{subtitles_language_code2_dot}}[\'"]?',
        str(language_code2).replace(":", "."),
        pp_command,
    )
    pp_command = _pp_sub(
        r'[\'"]?{{subtitles_language_code3_dot}}[\'"]?',
        str(language_code3).replace(":", "."),
        pp_command,
    )
    pp_command = _pp_sub(
        r'[\'"]?{{episode_language}}[\'"]?', str(episode_language), pp_command
    )
    pp_command = _pp_sub(
        r'[\'"]?{{episode_language_code2}}[\'"]?',
        str(episode_language_code2),
        pp_command,
    )
    pp_command = _pp_sub(
        r'[\'"]?{{episode_language_code3}}[\'"]?',
        str(episode_language_code3),
        pp_command,
    )
...
def set_chmod(subtitles_path):
    ...
    if chmod:
        logging.debug(
            f"BAZARR setting permission to {chmod} on {subtitles_path} after custom post-processing."  # noqa: G004
        )
