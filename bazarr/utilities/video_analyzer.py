    ...
    if cache_provider:
        for detected_language in data[cache_provider]["subtitle"]:
            ...
            if not language and und_default_language:
                logging.debug(
                    f"Undefined language embedded subtitles track treated as {language}"  # noqa: G004
                )
    ...
    if cache_provider:
        for detected_language in data[cache_provider]["audio"]:
            ...
            if isinstance(detected_language["language"], str):
                logging.error(
                    f"Cannot identify audio track language for this file: {file}. Value detected is "  # noqa: G004
    ...
    if cache_provider:
        ...
        try:
            ...
            for subtitles in parsed_subtitles:
                reversed_subtitles_path = (
                    path_mappings.path_replace_reverse(subtitles_path)
                    if sonarr_episode_id
                    else (path_mappings.path_replace_reverse_movie(subtitles_path))
                )
                    ...
                    references_dict["external_subtitles_tracks"].append(
                        {
                            ...
                            "path": path_mappings.path_replace(subtitles[1])
                            if sonarr_episode_id
                            else path_mappings.path_replace_reverse_movie(subtitles[1]),
...
def parse_video_metadata(
    ...
    if ffprobe_path:
        try:
            ...
            logging.error(
                f"BAZARR ffprobe cannot analyze this video file {file}. Could it be corrupted? {e}"  # noqa: G004
            )
            return None
    # or if we have mediainfo available
        ...
        try:
            ...
            logging.error(
                f"BAZARR mediainfo cannot analyze this video file {file}. Could it be corrupted? {e}"  # noqa: G004
            )
            return None
