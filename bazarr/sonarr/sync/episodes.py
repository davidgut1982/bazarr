...
def sync_episodes(series_id, defer_search=False, is_signalr=False, episodes_data=None):
    ...
    """
    logging.debug(
        "BAZARR Starting episodes sync from Sonarr for series ID %s.", series_id
    )
    ...
    if len(episodes_to_add):
        ...
        for chunk_start in range(0, len(episodes_to_add), EPISODE_INSERT_CHUNK_SIZE):
            chunk = episodes_to_add[
                chunk_start : chunk_start + EPISODE_INSERT_CHUNK_SIZE
            ]
            try:
                database.execute(insert(TableEpisodes).values(chunk))
            except IntegrityError as batch_err:
                logging.debug(
                    "BAZARR batched episode insert failed (%s); "
                    "falling back to per-row insert with update recovery",
                    batch_err,
                )
                for added_episode in chunk:
                    try:
                        ...
                        logging.error(
                            f"BAZARR cannot insert episodes because of {e}. We'll try to update it instead."  # noqa: G004
                        )
    ...
    if len(episodes_to_update):
        for updated_episode in episodes_to_update:
            ...
            cached_previous = current_episodes_in_db_row_as_dict.get(
                previous_episode_id, {}
            )
            ...
            try:
                ...
                if (
                    ...
                    logging.debug(
                        "BAZARR updating subtitles for episode %s",
                        updated_episode["path"],
                    )
                    store_subtitles(
                        updated_episode["path"],
                        path_mappings.path_replace(updated_episode["path"]),
                    )
                else:
                    logging.debug(
                        "BAZARR skipping subtitle update for episode %s as path "
                        "and episode_file_id unchanged",
                        updated_episode["path"],
                    )
                rows_changed = True
            ...
            for episode in episodes_to_update + episodes_to_add:
                ...
                if _is_there_missing_subtitles(episode_id=episode["sonarrEpisodeId"]):
                    if os.path.exists(path_mappings.path_replace(episode["path"])):
                        logging.debug(
                            "BAZARR downloading missing subtitles for this episode: %s",
                            episode_title,
                        )
                        ...
                        logging.debug(
                            "BAZARR cannot find this episode file yet (Sonarr may be slow to import episode "
                            "between disks?). Searching for missing subtitles is deferred until scheduled "
                            "task execution for this episode: %s",
                            episode_title,
                        )
                else:
                    if (
                    ...
                    logging.debug(
                        "BAZARR no missing subtitles for this episode: %s",
                        episode_title,
                    )
    ...
    logging.debug(
        "BAZARR All episodes from series ID %s synced from Sonarr into database.",
        series_id,
    )
...
def sync_one_episode(episode_id, defer_search=False, is_signalr=False):
    ...
    if not episode and existing_episode:
        try:
            ...
            logging.error(
                f"BAZARR cannot delete episode {existing_episode.path} because of {e}"  # noqa: G004
            )
        else:
            event_stream(type="episode", action="delete", payload=int(episode_id))
            logging.debug(
                "BAZARR deleted this episode from the database:%s",
    ...
    # Update existing episodes in DB
        ...
        try:
            ...
            logging.error(
                f"BAZARR cannot update episode {episode['path']} because of {e}"  # noqa: G004
            )
        else:
            store_subtitles(
                episode["path"], path_mappings.path_replace(episode["path"])
            )
            event_stream(type="episode", action="update", payload=int(episode_id))
            logging.debug(
                "BAZARR updated this episode into the database:%s",
    ...
    # Insert new episodes in DB
        ...
        try:
            ...
            logging.error(
                f"BAZARR cannot insert episode {episode['path']} because of {e}"  # noqa: G004
            )
        else:
            store_subtitles(
                episode["path"], path_mappings.path_replace(episode["path"])
            ...
            event_stream(type="episode", action="update", payload=int(episode_id))
            logging.debug(
                "BAZARR inserted this episode into the database:%s",
                path_mappings.path_replace(episode["path"]),
            )
    ...
    if defer_search:
        ...
        if os.path.exists(path_mappings.path_replace(episode["path"])):
            ...
            logging.debug(
                "BAZARR cannot find this file yet (Sonarr may be slow to import episode between disks?). "
