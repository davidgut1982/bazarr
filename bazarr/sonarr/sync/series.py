...
def update_series(job_id=None, wait_for_completion=False):
    ...
    try:
        ...
        current_shows_db = set(
            database.execute(select(TableShows.sonarrSeriesId)).scalars().all()
        )
        ...
        with ThreadPoolExecutor(
            max_workers=SONARR_PREFETCH_WORKERS,
            thread_name_prefix="bazarr-sonarr-prefetch",
        ) as executor:
            episode_futures = {
                show["id"]: executor.submit(
                    get_episodes_from_sonarr_api,
            ...
            for processed_index, (orig_index, show) in enumerate(
                shows_to_process, start=1
            ):
                jobs_queue.update_job_progress(
                    job_id=job_id,
                    progress_value=processed_index,
                ...
                try:
                    ...
                    logging.exception(
                        f"BAZARR error pre-fetching episodes for series {show['id']}"  # noqa: G004
                    )
                    episodes_data = None
...
def update_one_series(
    ...
    if action == "deleted" and existing_in_db:
        database.execute(
            delete(TableShows).where(TableShows.sonarrSeriesId == int(series_id))
        )
    ...
    if series_data is None:
        try:
            series_data_list = get_series_from_sonarr_api(
                apikey_sonarr=settings.sonarr.apikey, sonarr_series_id=int(series_id)
            )
        except Exception:
            logging.exception(
                f"BAZARR cannot get series with ID {series_id} from Sonarr API."  # noqa: G004
            )
            return
    ...
    if action == "updated" and existing_in_db:
        ...
        try:
            ...
            logging.error(
                f"BAZARR cannot update series {series['path']} because of {e}"  # noqa: G004
            )
        else:
            ...
            logging.debug(
                "BAZARR updated this series into the database:%s",
        ...
        try:
            ...
            logging.error(
                f"BAZARR cannot insert series {series['path']} because of {e}"  # noqa: G004
            )
        else:
            if not is_signalr and not skip_episode_sync:
            ...
            event_stream(type="series", action="update", payload=int(series_id))
            logging.debug(
                "BAZARR inserted this series into the database:%s",
                path_mappings.path_replace(series["path"]),
            )
